	...
	for (int i = 0; i <= MAX_HEIGHT; i++)
	{
		...
		for (int n = 0; n < N; n++)
		{
			for (int m = 0; m < M; m++)
			{
				...
				{
					int iHeightDif = ground[n][m] - i;
					iCurrentDuration += iHeightDif * 2;
					iCurrentB += iHeightDif;
				}
				// 쌓는 작업
				else
				{
					int iHeightDif = i - ground[n][m];
					iCurrentDuration += iHeightDif;
					iCurrentB -= iHeightDif;
				}
			}
		}
		...
		if (iDuration > iCurrentDuration)
		{
			iDuration = iCurrentDuration;
			iHeight = i;
		}
		else if (iDuration == iCurrentDuration)
		{
			iHeight = max(iHeight, i);
		}
	}

	cout << iDuration << ' ' << iHeight << '\n';
